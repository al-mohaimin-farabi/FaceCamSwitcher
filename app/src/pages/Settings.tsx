...
export default function Settings() {
  ...
  const { config, vmixTestResult } = useSelector(
  ...
  const handleTestVmix = async () => {
    ...
    try {
      ...
      dispatch(
        setVmixTestResult({ ok: result.success, message: result.message }),
      );
    } catch (e) {
  ...
  const saveSourcesNow = async (sources: string[]) => {
    ...
    const updated = {
      ...config,
      vmix: { ...config.vmix, target_sources: sources },
    };
    dispatch(
      updateConfigField({ path: "vmix.target_sources", value: sources }),
    );
    try {
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            >
              ...
              >
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateConfig("vmix.port", parseInt(e.target.value) || 8088)
                  }
                />
              </div>
              <div
              ...
              >
                ...
                <input
                  ...
                  type="number"
                  placeholder="8099"
                  value={config.vmix?.tcp_port ?? 8099}
                  onChange={(e) =>
                    updateConfig(
                      "vmix.tcp_port",
                      parseInt(e.target.value) || 8099,
                    )
                  }
                />
              </div>
            ...
            <div className="input-group">
              ...
              <input
                ...
                onChange={(e) =>
                  updateConfig("vmix.layer", parseInt(e.target.value) || 7)
                }
              />
            </div>
            ...
            <div>
              ...
              >
                {(config.vmix?.target_sources ?? []).length === 0 ? (
                  ...
                  >
                    No sources — add the vMix input names below
                  </span>
                ) : (
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input
                  className="input"
                  type="text"
                  placeholder='e.g. "Gameplay" or "OB 1"'
                  value={newSource}
            ...
            <button
              className="btn btn-primary"
              onClick={handleTestVmix}
              disabled={isTestingVmix}
              style={{
                height: 36,
                borderRadius: 8,
                fontSize: 12,
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            >
              ...
              >
                Debounce and clear-timeout are in milliseconds. Debounce
                prevents vMix spam when OCR text flickers. Clear-timeout sends
                SetLayer→None after the region goes blank.
              </p>
            </div>
          </div>
        </div>
