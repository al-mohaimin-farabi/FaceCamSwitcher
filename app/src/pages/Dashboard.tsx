...
export default function Dashboard() {
  ...
  const loadConfig = async () => {
    try {
      ...
      dispatch(addLog({ time: now(), level: "error", message: `Failed to load config: ${e}` }));
    }
  };
  ...
  const handleStartStop = async () => {
    if (isRunning) {
      try {
        ...
        dispatch(addLog({ time: now(), level: "error", message: `Failed to stop OCR: ${e}` }));
      }
    } else {
      try {
        ...
        dispatch(addLog({ time: now(), level: "error", message: `Failed to start OCR: ${e}` }));
      }
    }
  };
  ...
  const handleRefreshWindows = async () => {
    ...
    try {
      ...
      dispatch(addLog({ time: now(), level: "error", message: `Failed to list windows: ${e}` }));
    } finally {
  ...
  const handleRefreshCameras = async () => {
    ...
    try {
      ...
      dispatch(addLog({ time: now(), level: "error", message: `Failed to list cameras: ${e}` }));
    } finally {
  ...
  const handleSelectWindowRegion = async () => {
    ...
    try {
      ...
      dispatch(addLog({ time: now(), level: "error", message: `Region Selector Error: ${e}` }));
    } finally {
  ...
  const handleSelectCameraRegion = async () => {
    ...
    try {
      ...
      dispatch(addLog({ time: now(), level: "error", message: `Camera Region Selector Error: ${e}` }));
    } finally {
      setIsSelectingRegion(false);
    }
  };
  ...
  const handleSwitchSourceType = async (type: "window" | "camera") => {
    ...
    try {
      ...
      dispatch(addLog({ time: now(), level: "error", message: `Failed to switch source type: ${e}` }));
    }
  };
    ...
    <div style={{ display: "flex", height: "100%", padding: "16px", gap: "16px" }}>
      ...
      <div style={{ width: 340, flexShrink: 0, display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div className="glass-card" style={{ padding: 18 }}>
          ...
          {/* Source type toggle */}
          <div style={{ display: "flex", gap: 6, marginBottom: 12, padding: 3, background: "var(--bg-input)", borderRadius: 10, border: "1px solid var(--border)" }}>
            <button
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {srcType === "window" ? (
              ...
              <>
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  >
                    {cameraList.length === 0 ? (
                      ...
                      cameraList.map((c) => (
                        <option key={c.index} value={c.index}>{c.name} (index {c.index})</option>
                      ))
                    )}
                  </select>
                  <button
                ...
                {cameraList.length === 0 ? (
                  <p style={{ fontSize: 11, color: "var(--yellow)", marginTop: -2 }}>
            ...
            <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 12, background: "rgba(0,0,0,0.15)" }}>
              {previewData?.image ? (
                ...
                <div style={{ color: "var(--text-muted)", fontSize: 12 }}>
                  {isRunning ? "Waiting for frame..." : "Start capture to see preview"}
                </div>
              )}
            </div>
          </div>
              ...
              {previewData?.detections && previewData.detections.length > 0 ? (
                previewData.detections.map((d, i) => (
                  <div key={i} style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    ...
                    <div style={{ fontSize: 11, color: "var(--text-muted)", fontFamily: '"Cascadia Code", "Consolas", monospace' }}>
