...
import { Video, RefreshCw, Save, AlertCircle, CheckCircle2, Layers, Plus, X, Trash2 } from "lucide-react";
...
export default function Mapping() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      const extraKeys = Object.keys(map).filter((k) => !tagSet.has(k.toUpperCase()));
  ...
  const handleCameraChange = (tag: string, value: string) => {
    setRows((prev) => prev.map((r) => r.tag === tag ? { ...r, camera: value } : r));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showMessage(`Saved — ${mapped} / ${rows.length} team(s) mapped`, "success");
    } catch (e) {
  ...
  const handleClearAll = () => {
    if (!window.confirm("Clear all camera mappings (teams stay, camera inputs are cleared)?")) return;
    ...
    <div style={{ padding: 24, height: "100%", overflow: "auto", display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <div style={{
            display: "flex", gap: 8, padding: "5px 12px", borderRadius: 8,
            background: "var(--bg-secondary)", border: "1px solid var(--border)",
            fontSize: 12, fontWeight: 700, alignItems: "center",
          }}>
            <span style={{ color: "var(--green)" }}>{mappedCount} mapped</span>
            <span style={{ color: "var(--border)" }}>·</span>
            <span style={{ color: "var(--text-muted)" }}>{rows.length - mappedCount} unmapped</span>
          </div>
      ...
      <div style={{ padding: "10px 14px", borderRadius: 10, background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.15)", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <p style={{ fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
          OCR reads the screen text and fuzzy-matches it against your team tags (e.g. <strong style={{ color: "var(--accent)" }}>RHK</strong>). Enter the exact vMix input name on the right (e.g. <strong style={{ color: "var(--text-secondary)" }}>Camera 3</strong>). Use <strong style={{ color: "var(--text-secondary)" }}>Add Custom</strong> to add a new team+camera pair without switching to the Teams tab.
        </p>
      </div>
      ...
      <div className="glass-card" style={{ padding: 18 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 20px 1fr auto", gap: 10, alignItems: "center" }}>
          <input
            ...
            style={{ fontSize: 13 }}
          />
          <span style={{ fontSize: 18, fontWeight: 700, color: "var(--text-muted)", textAlign: "center" }}>→</span>
          <input
            className="input"
            type="text"
          ...
          <button
            ...
            style={{ height: 38, padding: "0 18px", borderRadius: 8, fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap" }}>
            <Plus size={13} /> Add
          </button>
        </div>
      </div>
      ...
      {/* ── Mapping Table ─────────────── */}
      <div className="glass-card" style={{ padding: 20, display: "flex", flexDirection: "column", gap: 12, flex: 1 }}>
        ...
        {isLoading ? (
          ...
          <div style={{ textAlign: "center", padding: "40px 0", background: "rgba(255,255,255,0.02)", borderRadius: 12 }}>
            ...
            <p style={{ marginTop: 12, color: "var(--text-muted)", fontSize: 13 }}>
              No teams yet — use the Add Custom form above or add teams in the Teams tab
            </p>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {/* Column headers */}
            <div style={{ display: "grid", gridTemplateColumns: "160px 24px 1fr 36px", gap: 10, padding: "0 10px", marginBottom: 2 }}>
            ...
            {rows.map(({ tag, camera }) => {
              ...
              return (
                <div key={tag} style={{
                  display: "grid", gridTemplateColumns: "160px 24px 1fr 36px",
                  gap: 10, alignItems: "center", padding: "8px 10px", borderRadius: 10,
                  background: isMapped ? "rgba(34,197,94,0.05)" : "rgba(255,255,255,0.02)",
                  ...
                  {/* Arrow */}
                  <span style={{ fontSize: 15, fontWeight: 700, color: isMapped ? "var(--green)" : "var(--border)", textAlign: "center" }}>
                    →
                  </span>
