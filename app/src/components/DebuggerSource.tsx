...
import { FolderSearch, FolderOpen, FolderCheck, FolderX, CheckCircle2, XCircle, Wand2 } from "lucide-react";
...
export default function DebuggerSource() {
  ...
  const folderLeaf = folderParts.length ? folderParts[folderParts.length - 1] : null;
  ...
  const autoDetect = async () => {
    ...
    if (res.detected) {
      ...
      setMsg("No debugger folder found on any drive. Pick it manually with Browse…");
      setBusy(false);
    }
  };
  ...
  return (
    <div className="glass-card" style={{ padding: 20 }}>
      ...
      <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 14 }}>
        Reads the latest file from <code>…\Free Fire_64_Data\Debugger</code> — auto-detect scans every drive.
      </div>
      ...
      <div className={`src-path ${folder ? "active" : ""}`}>
        ...
        <div style={{ minWidth: 0 }}>
          <div className="src-path-label">{folder ? "Selected folder" : "No folder selected"}</div>
          {folder ? (
            <>
              <div className="src-path-leaf">{folderLeaf}</div>
              <div className="src-path-dir" title={folder}>{folder}</div>
            </>
          ) : (
            <div className="src-path-dir" style={{ color: "var(--text-muted)" }}>
      ...
      {validation && (
        <div className="src-flags">
          ...
          <Flag ok={validation.hasLogFiles} label={`${validation.fileCount} log file${validation.fileCount === 1 ? "" : "s"}`} />
      ...
      {candidates.length > 0 && (
        <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid var(--border)" }}>
          <div className="section-header"><FolderSearch size={14} /> Found Locations</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {candidates.map((c) => (
              ...
              >
                <FolderOpen size={14} style={{ flexShrink: 0 }} /> <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c}</span>
              </button>
            ))}
          </div>
        </div>
